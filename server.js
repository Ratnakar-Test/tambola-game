// server.js
// Tambola Game Backend with Firestore Persistence (Diagnostic for Nested Array Issue)
    ...
    isOnline: true
                ...
                case 'ADMIN_APPROVE_TICKET_REQUEST': { 
                    ...
                    await db.runTransaction(async (transaction) => {
                        ...
                        transaction.set(db.collection('gameTickets').doc(ticketId), { userId: targetPlayerId, playerName: playerDoc.playerName, roomId, numbers: ticketNumbers, createdAt: FieldValue.serverTimestamp() });
                        const playerUpdatePath = `currentActivePlayers.${targetPlayerId}`;
                        transaction.update(roomRef, { 
                            [`${playerUpdatePath}.ticketCount`]: FieldValue.increment(1), 
                            [`${playerUpdatePath}.tickets`]: FieldValue.arrayUnion({ id: ticketId }) // Still only storing ID here
                        });
                    });
                ...
                case 'PLAYER_CLAIM_PRIZE': { 
                    ...
                    const roomData = roomDoc.data(); const ticketData = ticketDoc.data(); const ruleToClaim = (roomData.rules || []).find(r => r.id === prizeRuleId && r.isActive);
                    ...
                    const isValid = validatePrizeClaim(ticketData.numbers, roomData.currentNumbersCalled, ruleToClaim.name); const claimId = generateUniqueId();
                    const claimData = { userId: firebaseUID, playerName: ticketData.playerName, roomId, ticketId, prizeRuleId, prizeName: ruleToClaim.name, status: "pending_admin_approval", serverValidationResult: isValid, claimTimestamp: FieldValue.serverTimestamp()};
