...
// generateTambolaTicket and generateUniqueId removed as they were unused here.
// They are available in ./utils.js for other files that need them.
const {parseGameRulesFromString, generateRoomId} = require("./utils");
 ...
 */
exports.createGameRoomHandler = (db, adminInstance) => async (data, context) => {
  if (!context.auth) {
  ...
  const roomData = {
    ...
    currentActivePlayers: {
      [adminUID]: {
        ...
        ticketCount: 0,
        lastSeen: adminInstance.firestore.FieldValue.serverTimestamp(),
        isAdmin: true,
      },
    },
    currentWinners: [],
  ...
  try {
    ...
    console.log(
      `Room ${newRoomId} created by admin ${adminUID} (${adminDisplayName})`,
    );
 ...
 * @return {Function} Callable Cloud Function.
 */
exports.joinGameRoomHandler = (db, adminInstance) => async (data, context) => {
  if (!context.auth) {
    throw new HttpsError(
      "unauthenticated",
  ...
  try {
    ...
    const playerData = {
      ...
      ticketCount: 0,
      lastSeen: adminInstance.firestore.FieldValue.serverTimestamp(),
      isAdmin: roomDoc.data().adminUID === playerUID,
    };
...
exports.setGameRulesHandler = (db) => async (data, context) => {
  ...
  const adminUID = context.auth.uid;
  const {roomId, rulesString} = data;

  if (!roomId) {
    throw new HttpsError("invalid-argument", "Room ID is required.");
  }
  if (!rulesString) {
    throw new HttpsError("invalid-argument", "Rules string is required.");
  }
  ...
  if (roomDoc.data().adminUID !== adminUID) {
    throw new HttpsError(
      "permission-denied",
      "Only the room admin can set rules.",
    );
  }
  if (roomDoc.data().gameStatus !== "idle" && roomDoc.data().gameStatus !== "stopped") {
    throw new HttpsError(
      "failed-precondition",
      "Rules can only be set when the game is idle or stopped.",
    );
  }

  const parsedRules = parseGameRulesFromString(rulesString);
  if (!parsedRules) {
    throw new HttpsError(
      "invalid-argument",
      "Invalid rules format. Expected 'Name:Percentage,...'. " +
      "Percentages must be >0 and <=100.",
    );
  }
  const totalPercentage = parsedRules.reduce(
  ...
  if (totalPercentage > 100) {
    throw new HttpsError(
      "invalid-argument",
      `Total prize percentage (${totalPercentage}%) cannot exceed 100%.`,
    );
  }
 ...
 * @return {Function} Callable Cloud Function.
 */
exports.startGameHandler = (db, adminInstance) => async (data, context) => {
  if (!context.auth) {
    throw new HttpsError("unauthenticated", "User must be authenticated.");
  }
  const adminUID = context.auth.uid;
  ...
  if (roomData.gameStatus !== "idle" && roomData.gameStatus !== "stopped") {
    throw new HttpsError(
      "failed-precondition",
      `Game cannot be started. Current status: ${roomData.gameStatus}.`,
    );
  }
  if (!roomData.rules || roomData.rules.length === 0) {
    throw new HttpsError(
      "failed-precondition",
      "Cannot start game. Please set game rules first.",
    );
  }
  ...
  try {
    await roomRef.update({
      gameStatus: "running",
      gameStartTime: adminInstance.firestore.FieldValue.serverTimestamp(),
      gameEndTime: null,
      currentNumbersCalled: [],
      currentLatestCalledNumber: null,
      latestCalledPhrase: "",
 ...
 * @return {Function} Callable Cloud Function.
 */
exports.stopGameHandler = (db, adminInstance) => async (data, context) => {
  if (!context.auth) {
    throw new HttpsError("unauthenticated", "User must be authenticated.");
  }
  const adminUID = context.auth.uid;
  ...
  if (roomData.adminUID !== adminUID) {
    throw new HttpsError(
      "permission-denied",
      "Only the room admin can stop the game.",
    );
  }
  if (roomData.gameStatus === "idle" || roomData.gameStatus === "stopped") {
    throw new HttpsError(
      "failed-precondition",
      `Game is already ${roomData.gameStatus}.`,
    );
  }
  ...
  try {
    const gameSummary = {
      ...
      winnersSnapshot: [...roomData.currentWinners],
      playersSnapshot: {...roomData.currentActivePlayers},
      rulesSnapshot: [...roomData.rules],
      totalMoneyCollected: roomData.totalMoneyCollected,
      gameStartTime: roomData.gameStartTime,
      gameEndTime: adminInstance.firestore.FieldValue.serverTimestamp(),
    };
    ...
    console.log(
      `Game stopped in room ${roomId} by admin ${adminUID}. Summary generated.`,
    );
...
exports.setCallingModeHandler = (db) => async (data, context) => {
  ...
  const adminUID = context.auth.uid;
  const {roomId, mode, interval} = data;
  ...
  if (!["manual", "auto"].includes(mode)) {
    throw new HttpsError(
      "invalid-argument",
      "Invalid calling mode. Must be 'manual' or 'auto'.",
    );
  }
  if (mode === "auto" && (typeof interval !== "number" || interval < 3 || interval > 60) ) {
    throw new HttpsError(
      "invalid-argument",
      "Auto-call interval must be a number between 3 and 60 seconds.",
    );
  }
  ...
  if (roomDoc.data().adminUID !== adminUID) {
    throw new HttpsError(
      "permission-denied",
      "Only the room admin can set the calling mode.",
    );
  }
  ...
  try {
    ...
    console.log(
      `Calling mode for room ${roomId} set to ${mode} ` +
      `(Interval: ${interval || "N/A"}s) by admin ${adminUID}`,
    );
...
exports.togglePauseResumeGameHandler = (db) => async (data, context) => {
  ...
  const adminUID = context.auth.uid;
  const {roomId, targetStatus} = data;
  ...
  if (roomData.adminUID !== adminUID) {
    throw new HttpsError(
      "permission-denied",
      "Only the room admin can pause/resume the game.",
    );
  }
  ...
  if (targetStatus === "paused" && roomData.gameStatus !== "running") {
    throw new HttpsError(
      "failed-precondition",
      "Game must be 'running' to be paused.",
    );
  }
  if (targetStatus === "running" && roomData.gameStatus !== "paused") {
    throw new HttpsError(
      "failed-precondition",
      "Game must be 'paused' to be resumed.",
    );
  }
  ...
  try {
    ...
    console.log(
      `Game in room ${roomId} status changed to ${targetStatus} by admin ${adminUID}`,
    );
...
/**
 * Updates a player's last seen timestamp.
 * @param {FirebaseFirestore.Firestore} db - Firestore database instance.
 * @param {object} adminInstance - Firebase Admin SDK instance.
 * @return {Function} Callable Cloud Function.
 */
exports.updatePlayerPresenceHandler = (db, adminInstance) => async (data, context) => {
  if (!context.auth) {
  ...
  try {
    ...
    await roomRef.update({
      [playerPath]: adminInstance.firestore.FieldValue.serverTimestamp(),
    });
    return {success: true};
  } catch (error) {
    console.warn(
      `Could not update presence for player ${playerUID} in room ${roomId}:`,
      error.message,
    );
